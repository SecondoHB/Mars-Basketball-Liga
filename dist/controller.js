...
export function bindTeamOneInput() {
    dom.teamOneSearch.addEventListener("input", debounce((e) => {
        const target = e.target;
        const cleaned = target.value.replace(/[^A-Za-zÄÖÜäöüß0-9\s]/g, "");
        if (cleaned !== target.value) {
            target.value = cleaned;
            target.setCustomValidity("Nur Buchstaben, Zahlen und Leerzeichen erlaubt");
...
export function bindTeamTwoInput() {
    dom.teamTwoSearch.addEventListener("input", debounce((e) => {
        const target = e.target;
        const cleaned = target.value.replace(/[^A-Za-zÄÖÜäöüß0-9\s]/g, "");
        if (cleaned !== target.value) {
            target.value = cleaned;
